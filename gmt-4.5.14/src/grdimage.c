/*--------------------------------------------------------------------
 *	$Id: grdimage.c 10314 2015-06-11 06:18:34Z pwessel $
 *
...
int main (int argc, char **argv)
{
...
#endif
	if (n_grids == 0) {
		fprintf (stderr, "%s: GMT SYNTAX ERROR:  Must specify 1 or 3 input grids\n", GMT_program);
		error++;
	}
	...
	if (need_to_project) {	/* Need to resample the grd file */
		...
		for (i = 0; i < n_grids; i++) {
			...
			nm2 = GMT_get_nm (4 + r_head[i].nx, 4 + r_head[i].ny);
