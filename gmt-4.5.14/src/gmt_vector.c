/*--------------------------------------------------------------------
 *	$Id: gmt_vector.c 10289 2014-12-28 21:17:06Z pwessel $
 *
...
/*                                               multiple systems with same a */
        static GMT_LONG l1;
        GMT_LONG *line = NULL, *isub = NULL, i = 0, j, k, l, j2, n, nstore;
	GMT_LONG iet, ieb;
...
GMT_LONG GMT_fix_up_path (double **a_lon, double **a_lat, GMT_LONG n, double step, GMT_LONG mode)
{
	...
	GMT_LONG meridian;
	double *lon_tmp = NULL, *lat_tmp = NULL, *old = NULL;
	...
	GMT_eliminate_lon_jumps (lon_tmp, n_tmp);
	old = lon;
	lon = lon_tmp;
	GMT_free ((void *) old);
	old = lat;
