/*--------------------------------------------------------------------
 *	$Id: gmt_grdio.c 10307 2015-04-28 20:25:58Z pwessel $
 *
...
{	/* file:	- IGNORED - info already in header
	 * header:	grid structure header
	...
	if (complex) nm *= 2;	/* Twice as many nodes to scale */
...
{	/* file:	File name
	...
	GMT_grd_set_units (header);
	nm = ((GMT_LONG)(header->nx + pad[0] + pad[1])) * ((GMT_LONG)(header->ny + pad[2] + pad[3]));
	if (complex) nm *= 2;	/* Twice as many nodes */
...
GMT_LONG GMT_grd_RI_verify (struct GRD_HEADER *h, GMT_LONG mode)
{
	...
	switch (GMT_minmaxinc_verify (h->x_min, h->x_max, h->x_inc, GMT_SMALL)) {
		...
		case 2:
			...
			if (GMT_io.in_col_type[0] == GMT_IS_LON) (void) fprintf (stderr, "%s: GMT ERROR: Make sure west < east for geographic coordinates\n", GMT_program);
			error++;
			break;
		case 1:
...
GMT_LONG GMT_open_grd (char *file, struct GMT_GRDFILE *G, char mode)
{
	...
	if (GMT_grdformats[G->header.type][0] == 'c') {		/* Open netCDF file, old format */
		...
		if (header) GMT_cdf_grd_info (G->fid, &G->header, mode);
...
{	/* GMT_grd_init initializes a grd header to default values and copies the
	...
	if (argc > 0) {
		...
		for (i = 1; len < GRD_COMMAND_LEN && i < argc; i++) {
			len += strlen (argv[i]) + 1;	/* Space plus arg */
...
GMT_LONG GMT_adjust_loose_wesn (double *w, double *e, double *s, double *n, struct GRD_HEADER *header)
{
	...
	if (!global) {
		if (GMT_io.in_col_type[0] == GMT_IS_LON) {
			if (header->x_max < *w)
				header->x_min += 360.0, header->x_max += 360.0;
			else if (header->x_min > *e)
				header->x_min -= 360.0, header->x_max -= 360.0;
		}
		if (header->x_min - *w > GMT_SMALL) { *w = header->x_min; error = TRUE; }
		if (*e - header->x_max > GMT_SMALL) { *e = header->x_max; error = TRUE; }
	}
	if (header->y_min - *s > GMT_SMALL) { *s = header->y_min; error = TRUE; }
	if (*n - header->y_max > GMT_SMALL) { *n = header->y_max; error = TRUE; }
	if (error) fprintf (stderr, "%s: Warning: Region exceeds grid domain. Region reduced to grid domain.\n", GMT_program);
	error = FALSE;
