/*--------------------------------------------------------------------
 *    $Id: psmeca.c 10331 2015-10-29 19:41:56Z pwessel $
 *
...
int main (int argc, char **argv)
{
    ...
    for (fno = 1; !done && fno < n_args; fno++) {    /* Loop over all input files */
	    ...
	    /* Immediately skip locations outside of the map area */
            ...
            if ((GMT_scanf (col[0], GMT_io.in_col_type[ix], &xy[ix]) == GMT_IS_NAN) || (GMT_scanf (col[1], GMT_io.in_col_type[iy], &xy[iy]) == GMT_IS_NAN)) {
                fprintf (stderr, "%s: Record had bad x and/or y coordinates, must exit)\n", GMT_program);
	        exit (EXIT_FAILURE);
            }
...
/* normalization by M0 */
	    ...
	    /* If option -C is used, read the new position */
            ...
            if (read_change_position) {
 	        if ((GMT_scanf (col[last-1+new], GMT_io.in_col_type[ix], &xynew[ix]) == GMT_IS_NAN) || (GMT_scanf (col[last+new], GMT_io.in_col_type[iy], &xynew[iy]) == GMT_IS_NAN)) {
	            fprintf (stderr, "%s: Record had bad newX and/or newYcoordinates, must exit\n", GMT_program);
		    exit (EXIT_FAILURE);
	        }
