/*--------------------------------------------------------------------
 *	$Id: gmt_shore.c 10289 2014-12-28 21:17:06Z pwessel $
 *
...
GMT_LONG GMT_init_shore (char res, struct GMT_SHORE *c, double w, double e, double s, double n, struct GMT_SHORE_SELECT *info) {	/* res: Resolution (f, h, i, l, c */
	...
	/* Open shoreline file */
	if ((err = nc_open (path, NC_NOWRITE, &c->cdfid)) != GMT_NOERROR) {
		fprintf (stderr, "Unable to open %s. Make sure you have netCDF library version >= 4.0, with HDF5 support\n", path);
		return (err);
	}
	...
	/* Get global attributes */
        if (nc_get_att_text (c->cdfid, NC_GLOBAL, "version", c->version) ||
			...
			fprintf (stderr, "GSHHG: ftp://ftp.soest.hawaii.edu/gshhg/gshhg-gmt-%s.tar.gz\n", GSHHG_VERSION);
	...
	c->GSHHS_area = (double *) GMT_memory (VNULL, (size_t)c->n_poly, sizeof (double), "GMT_init_shore");
...
/* max_level: Polygons with higher levels are ignored */
{
	...
	GMT_LONG s, i, k, err, level, inc[4], ll_node, node, ID;
	double w, e, dx;
	...
	for (i = 0; i < c->bin_nseg[b]; i++) {
		...
		if (level == 2 && c->GSHHS_area[seg_ID[i]] < 0.0 && c->flag == GMT_NO_RIVERLAKES) continue;
		if (level == 2 && c->GSHHS_area[seg_ID[i]] > 0.0 && c->flag == GMT_NO_LAKES) continue;
	...
	for (s = 0; s < c->ns; s++) {
		...
		c->seg[s].fid = (c->GSHHS_area[seg_ID[s]] < 0.0) ? RIVERLAKE : c->seg[s].level;
