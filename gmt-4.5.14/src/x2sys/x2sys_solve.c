/*-----------------------------------------------------------------
 *	$Id: x2sys_solve.c 10289 2014-12-28 21:17:06Z pwessel $
 *
...
int main (int argc, char **argv)
{
	...
	GMT_LONG i, p, j, k, r, s, off, row, n_COE = 0, n_alloc = GMT_CHUNK, n_alloc_t = GMT_CHUNK, ierror, *R = NULL, *col_off = NULL;
	double *N = NULL, *a = NULL, *b = NULL, *data[N_COE_PARS], sgn, zero_test = 1.0e-08, old_mean, new_mean, sw2, var[N_BASIS];
	double old_stdev, new_stdev, e_k, Sw, Sx, Sxx, C_i, C_j, range = 0.0, *start = NULL;
	double sc_range = 1;
	...
	R = (GMT_LONG *)GMT_memory (NULL, (size_t)n_tracks, sizeof (GMT_LONG), GMT_program);		/* Number of parameter for each track p, so that 1 <= R(p) < n_par */
	col_off = (GMT_LONG *)GMT_memory (NULL, (size_t)n_tracks, sizeof (GMT_LONG), GMT_program);	/* Since R(p) may vary, need cumulative sum of R(p) to get correct column */
	...
	for (p = n = 0; p < n_tracks; p++) {	/* For each track, determine R[track], total number of parameters, and the column offsets */
		(GMT_io.binary[GMT_IN]) ? sprintf (trk[0], "%ld", p) : sprintf (trk[0], "%s", trk_list[p]);
		if (R[p] < n_par)	/* Came up short */
			fprintf (stderr, "%s: Warning: Track %s only has %ld crossings so can only solve for %ld of the %ld parameters\n", GMT_program, trk[0], R[p], R[p], n_par);
		else
			R[p] = n_par;
	...
	fprintf (stderr, "%s: Number of unknowns are %ld [+1 if model contains a constant]\n", GMT_program, n);
	...
	for (p = row = 0; p < n_tracks; p++) {	/* For each track */
		for (s = 0; s < R[p]; s++, row++) {	/* For each track's unknown parameter  */
			for (k = 0; k < n_COE; k++) {	/* For each crossover */
				...
				for (r = 0, off = m * row; r < R[i]; r++)	/* For each track's parameter in f(p)  */
					N[off+col_off[i]+r] += sw2 * (basis[r](data,0,k) * basis[s](data,t,k));
				for (r = 0, off = m * row; r < R[j]; r++)	/* For each track's parameter in f(p)  */
					N[off+col_off[j]+r] -= sw2 * (basis[r](data,1,k) * basis[s](data,t,k));
	...
	if (mode != F_IS_SCALE) {	/* Augmented row for Lagrange multiplier for constants */
		for (i = 0, off = m*n; i < n_tracks; i++) N[off+col_off[i]] = 1.0;
	}
	...
	if (ierror) {
		fprintf (stderr, "%s: Warning: Divisions by a small number (< DBL_EPSILON) occurred in GMT_gauss()!\n", GMT_program);
		fprintf (stderr, "%s: This probably means your tracks form more than one independent cluster.\n", GMT_program);
		fprintf (stderr, "%s: Either identify these clusters and run x2sys_solve separately on each one or upgrade to GMT 5.\n", GMT_program);
	}
	...
	new_stdev = sqrt ((n_COE * Sxx - Sx * Sx) / (Sw*Sw*(n_COE - 1.0)/n_COE));
		...
		if (it > 0) {
			for (p = 0; p < n_tracks; p++)		/* For each track */ 
				for (r = 0; r < R[p]; r++) 	/* For each track's parameter in f(p)  */
					a[col_off[p] + r] += a[col_off[p] + r];
		}
	}	/* end iteration loop */
	...
	for (p = 0; p < n_tracks; p++) {
		...
		printf ("\t%s", column);
		memset ((void *)var, 0, N_BASIS * sizeof (double));	/* Reset all parameters to zero */
