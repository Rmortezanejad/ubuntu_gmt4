/*--------------------------------------------------------------------
 *	$Id: gmt2kml.c 10289 2014-12-28 21:17:06Z pwessel $
 *
 *	Copyright (c) 2009-2015 by P. Wessel
...
int main (int argc, char **argv)
{
	...
	GMT_LONG lod_min = 0, lod_max = 0, set_nr = 0, pnt_nr = 0, index = -4, pid;
	...
	printf ("\t<Style id=\"GMT-%ld-0\">\n", pid);	/* The default style */
	...
	for (fno = 1; !done && fno < n_args; fno++) {	/* Loop over input files, if any */
		...
		if (get_label) {	/* Special ascii table processing */
			while (GMT_fgets (buffer, BUFSIZ, fp)) {
				...
				printf ("\t\t<styleUrl>#GMT-%ld-%ld</styleUrl>\n", pid, index+4);
				printf ("\t\t<%s>\n", feature[fmode]);
