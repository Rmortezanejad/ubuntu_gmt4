/*--------------------------------------------------------------------
 *	$Id: gmt_io.c 10289 2014-12-28 21:17:06Z pwessel $
 *
...
char *GMT_getuserpath (const char *stem, char *path)
{
	...
	if (GMT_TMPDIR) {
		sprintf (path, "%s/%s", GMT_TMPDIR, stem);
	...
	if (GMT_HOMEDIR) {
		sprintf (path, "%s/%s", GMT_HOMEDIR, stem);
		if (!access (path, R_OK)) return (path);
	}
	if (GMT_USERDIR) {
		sprintf (path, "%s/%s", GMT_USERDIR, stem);
		if (!access (path, R_OK)) return (path);
	}
...
char *GMT_getdatapath (const char *stem, char *path)
{
	...
	if (GMT_GRIDDIR) {
		sprintf (path, "%s/%s", GMT_GRIDDIR, stem);
		if (!access (path, R_OK)) return (path);
	}
	if (GMT_IMGDIR) {
		sprintf (path, "%s/%s", GMT_IMGDIR, stem);
		if (!access (path, R_OK)) return (path);
	}
...
char *GMT_getsharepath (const char *subdir, const char *stem, const char *suffix, char *path)
{
	...
	if (GMT_USERDIR) {
		sprintf (path, "%s/%s%s", GMT_USERDIR, stem, suffix);
		if (!access (path, R_OK)) return (path);
	}
	...
	if (subdir) {
		sprintf (path, "%s/%s/%s%s", GMT_SHAREDIR, subdir, stem, suffix);
		if (!access (path, R_OK)) return (path);
	}

	/* Finally try file in $GMT_SHAREDIR (for backward compatibility) */

	sprintf (path, "%s/%s%s", GMT_SHAREDIR, stem, suffix);
	if (!access (path, R_OK)) return (path);
	...
	while (!done) {	/* Done becomes TRUE when we successfully have read a data record */
		...
		if (line[0] == GMT_io.EOF_flag[GMT_IN]) {	/* Got a multisegment header, take action and return */
			if (GMT_io.multi_segments[GMT_IN]) {
				...
				fprintf (stderr, "%s: ERROR: Input data have multiple segment headers but either the -m switch was not set or it is not supported by this program.\n", GMT_program);
				GMT_exit (EXIT_FAILURE);
			}
		}
...
void GMT_get_lon_minmax (double *lon, GMT_LONG n_rows, double *min, double *max)
{	/* Return the min/max longitude in array lon using clever quadrant checking. */
	GMT_LONG way, row;
	struct GMT_QUAD *Q = GMT_quad_init (1);	/* Allocate and initialize one QUAD structure */

	/* We must keep separate min/max for both Dateline and Greenwich conventions */
	for (row = 0; row < n_rows; row++) GMT_quad_add (Q, lon[row]);
...
GMT_LONG GMT_import_table (void *source, GMT_LONG source_type, struct GMT_TABLE **table, double dist, GMT_LONG greenwich, GMT_LONG poly, GMT_LONG use_GMT_io)
{
	...
	for (seg = 0; seg < T->n_segments; seg++) {
		for (col = 0; col < T->n_columns; col++) {
			if (GMT_io.in_col_type[col] & GMT_IS_LON) {	/* Update longitude ranges */
				double min, max;
				GMT_get_lon_minmax (T->segment[seg]->coord[col], T->segment[seg]->n_rows, &min, &max);
				T->segment[seg]->min[col] = min;
				T->segment[seg]->max[col] = max;
			}
...
{	/* Allocate and initialize the QUAD struct needed to find min/max of a set of longitudes */
	...
	memset ((void *)Q, 0, (size_t)(n_items * sizeof (struct GMT_QUAD)));	/* Set all to NULL/0 */
	for (i = 0; i < n_items; i++) {
...
struct GMT_QUAD * GMT_quad_init (GMT_LONG n_items)
{	/* Allocate an initialize the QUAD struct needed to find min/max of longitudes */
	struct GMT_QUAD *Q = (struct GMT_QUAD *)GMT_memory (VNULL, (size_t)n_items, sizeof (struct GMT_QUAD), GMT_program);
...
void GMT_quad_add (struct GMT_QUAD *Q, double x)
...
GMT_LONG GMT_quad_finalize (struct GMT_QUAD *Q)
{
	/* Finalize longitude range settings */
	GMT_LONG way, n_quad;
...
void GMT_eliminate_lon_jumps (double *lon, GMT_LONG n_rows)
