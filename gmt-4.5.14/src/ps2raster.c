/*--------------------------------------------------------------------
 *	$Id: ps2raster.c 10320 2015-08-06 07:53:05Z remko $
 *
 *	Copyright (c) 2006-2015 by J. Luis
...
int main (int argc, char **argv) {
	...
	GMT_LONG got_BB, got_HRBB, got_BBatend, file_has_HRBB, got_end, landscape, setup;
	int sys_retval = 0;
	double xt, yt, w, h, x0 = 0.0, x1 = 612.0, y0 = 0.0, y1 = 828.0;
	double	west = 0.0, east = 0.0, south = 0.0, north = 0.0;
#ifdef USE_GDAL
	double	west_prj = 0.0, south_prj = 0.0, east_prj = 0.0, north_prj = 0.0;
#endif
	...
	for (k = 0; k < n_files; k++) {
		...
		if (Ctrl->A.active) {
			...
			if (sys_retval) {
				fprintf (stderr, "%s: System call [%s] returned error %d.\n", GMT_program, cmd, sys_retval);
				exit (EXIT_FAILURE);
			}
