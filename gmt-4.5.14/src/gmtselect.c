/*--------------------------------------------------------------------
 *	$Id: gmtselect.c 10289 2014-12-28 21:17:06Z pwessel $
 *
...
{
	GMT_LONG i, j, k, fno, n_files = 0, n_args, err = 0, n_minimum = 2;
	...
	if (Ctrl->N.active) {
		...
		if ((err = GMT_init_shore (Ctrl->D.set, &c, west, east, south, north, &Ctrl->A.info))) {
			fprintf (stderr, "%s: %s [GSHHG %s resolution shoreline]\n", GMT_program, GMT_strerror(err), shore_resolution[base]);
			exit (EXIT_FAILURE);
		}
	...
	for (fno = 1; !done && fno < n_args; fno++) {	/* Loop over input files, if any */
		...
		while ((n_fields = GMT_input (fp, &n_expected_fields, &in)) >= 0 && ! (GMT_io.status & GMT_IO_EOF)) {	/* Not yet EOF */
			...
			if (Ctrl->Z.active) {	/* NaN is considered inside */
				inside = (GMT_is_dnan (in[GMT_Z]) || (in[GMT_Z] >= Ctrl->Z.min && in[GMT_Z] <= Ctrl->Z.max)); 
