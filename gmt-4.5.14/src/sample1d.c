/*--------------------------------------------------------------------
 *	$Id: sample1d.c 10289 2014-12-28 21:17:06Z pwessel $
 *
	...
	for (fno = 1; !done && fno < n_args; fno++) {	/* Loop over input files, if any */
		...
		while (! (GMT_io.status & GMT_IO_EOF)) {	/* Not yet EOF */
			...
			if (Ctrl->N.active) {	/* Get relevant t_out segment */
				...
				GMT_LONG first_k, last_k;
				double abs_inc, last_t;
				...
				first_k = (Ctrl->I.inc > 0.0) ? 0 : n-1;	/* Index of point with earliest time */
				last_k  = (Ctrl->I.inc > 0.0) ? n-1 : 0;	/* Index of point with latest time */
				abs_inc = fabs (Ctrl->I.inc);
				if (!Ctrl->S.active) {
					Ctrl->S.start = floor (col[Ctrl->T.col][first_k] / abs_inc) * abs_inc;
					if (Ctrl->S.start < col[Ctrl->T.col][first_k]) Ctrl->S.start += abs_inc;
				}
				last_t = col[Ctrl->T.col][last_k];
				/* So here, Ctrl->S.start holds to smallest t value and last_t holds the largest t_value, regardless of inc sign */
				m = m_alloc = irint (fabs((last_t - Ctrl->S.start) / abs_inc)) + 1;
				...
				if (Ctrl->I.inc > 0.0) {
					...
					if (fabs (t_out[i-1]-last_t) < GMT_SMALL) t_out[i-1] = last_t;	/* Fix roundoff */
				}
				else {
					t_out[0] = last_t;
					while (i < m && (tt = last_t + i * Ctrl->I.inc) >= Ctrl->S.start) {
						t_out[i] = tt;
						i++;
					}
					if (fabs (t_out[i-1]-Ctrl->S.start) < GMT_SMALL) t_out[i-1] = Ctrl->S.start;	/* Fix roundoff */
				}
				m = i;
			}
