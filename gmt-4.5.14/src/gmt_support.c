/*--------------------------------------------------------------------
 *	$Id: gmt_support.c 10325 2015-10-14 20:37:49Z pwessel $
 *
...
void GMT_flip_angle_f (float *angle)
{
...
void GMT_flip_angle_d (double *angle)
{
	if (project_info.projection == GMT_LINEAR) {	/* Must check if negative scales were used */
		if (!project_info.xyz_pos[0]) {	/* Negative x scale */
			if (!project_info.xyz_pos[1])	/* Negative y-scale too */
...
void GMT_enforce_rgb_triplets (char *text, GMT_LONG size)
{
	...
	int rgb[3];
	char buffer[BUFSIZ], color[16], *p = NULL;
...
void GMT_RI_prepare (struct GRD_HEADER *h)
{
	...
	*/
	...
	if (GMT_inc_code[0] & GMT_INC_IS_NNODES) {	/* Got nx */
		...
		h->x_inc *= s / (project_info.DIST_M_PR_DEG * cosd (0.5 * (h->y_min + h->y_max)));	/* Latitude scaling of E-W distances */
	...
	if (GMT_inc_code[1] & GMT_INC_IS_NNODES) {	/* Got ny */
		...
		h->y_inc = (h->y_inc == 0.0) ? h->x_inc : h->y_inc * s / project_info.DIST_M_PR_DEG;
...
void GMT_sample_cpt (double z[], GMT_LONG nz, GMT_LONG continuous, GMT_LONG reverse, GMT_LONG log_mode)
{
	...
	double *x = NULL, *z_out = NULL, a, b, f, x_inc, cmyk_low[4], cmyk_high[4];
...
GMT_LONG GMT_contlabel_info (char flag, char *txt, struct GMT_CONTOUR *L)
{
	...
	char txt_a[GMT_LONG_TEXT], c, *p = NULL;
	...
	if (ctype == 'A' || ctype == 'a') {	/* Annotated contours, must find label placement */
		...
		if (G->crossing) {	/* Determine label positions based on crossing lines */
			...
			for (line_no = 0; line_no < G->xp->n_segments; line_no++) {	/* For each of the crossing lines */
				...
				G->nx = GMT_crossover (G->xp->segment[line_no]->coord[GMT_X], G->xp->segment[line_no]->coord[GMT_Y], NULL, G->ylist_XP, G->xp->segment[line_no]->n_rows, xx, yy, NULL, G->ylist, nn, FALSE, FALSE, &G->XC);
...
{	/* Allocates needed space and copies in the label */
	GMT_LONG n, m = 0;
	if (use_unit && G->unit[0]) m = strlen (G->unit) + (G->unit[0] != '-');	/* Must allow extra space for a unit string */
	n = strlen (txt) + 1 + m;
...
{	/* Case of a polar cap */
	...
	for (i = count[0] = count[1] = 0; i < P->n_rows - 1; i++) {	/* -1, since we know last point repeats the first */
		...
		if (GMT_SAME_LONGITUDE (plon, lon1)) {	/* Line goes through the 1st node */
			...
			if (dx1 == 0.0) continue;	/* Points ip and i forms a meridian, we have a tangent line */
			dx2 = lon2 - lon1;
...
double GMT_flatearth_dist_meter (double x0, double y0, double x1, double y1)
{
	...
	return (hypot ( dlon * cosd (0.5 * (y1 + y0)), (y1 - y0)) * project_info.DIST_M_PR_DEG);
}
...
double GMT_great_circle_dist_km (double x0, double y0, double x1, double y1)
{
	...
	return (GMT_great_circle_dist (x0, y0, x1, y1) * project_info.DIST_KM_PR_DEG);
}
...
double GMT_great_circle_dist_meter (double x0, double y0, double x1, double y1)
{
	...
	return (GMT_great_circle_dist (x0, y0, x1, y1) * project_info.DIST_M_PR_DEG);
}
...
GMT_LONG GMT_near_a_line_spherical (double lon, double lat, struct GMT_TABLE *T, GMT_LONG return_mindist, double *dist_min, double *x_near, double *y_near)
{
	...
	for (seg = 0; seg < T->n_segments; seg++) {	/* Loop over each line segment */
		...
		cos_dist = (return_mindist) ? 2.0 : cosd (T->segment[seg]->dist / project_info.DIST_KM_PR_DEG);		/* Cosine of the great circle distance we are checking for. 2 ensures failure to be closer */
...
GMT_LONG gmt_x_overlap (double *xa, double *xb, GMT_LONG *xa_start, GMT_LONG *xa_stop, GMT_LONG *xb_start, GMT_LONG *xb_stop, GMT_LONG geo)
{
	...
	if (geo) {	/* More complicated, and may change both the start/stop indices and the array longitudes */
		...
		if (dx > 180.0) {xa[*xa_start] += 360.0; l_swap(*xa_start, *xa_stop);}	/* Deal with 360 and swap start and stop indices */
		dx = xb[*xb_stop] - xb[*xb_start];
		if (dx > 180.0) {xb[*xb_start] += 360.0; l_swap(*xb_start, *xb_stop);}	/* Deal with 360 and swap start and stop indices */
...
GMT_LONG GMT_crossover (double xa[], double ya[], GMT_LONG *sa0, struct GMT_XSEGMENT A[], GMT_LONG na, double xb[], double yb[], GMT_LONG *sb0, struct GMT_XSEGMENT B[], GMT_LONG nb, GMT_LONG internal, GMT_LONG geo, struct GMT_XOVER *X)
{
	...
	*distance_func = (GMT_IS_SPHERICAL) ? GMT_great_circle_dist : GMT_geodesic_dist_meter;
	switch (c) {
		...
		case 'e':
			...
			*d_scale = project_info.DIST_M_PR_DEG;
			break;
		case 'E':	/* m along geodesic */
			*distance_func = (GMT_IS_SPHERICAL) ? GMT_great_circle_dist : GMT_geodesic_dist_meter;
			*d_scale = (GMT_IS_SPHERICAL) ? project_info.DIST_M_PR_DEG : 1.0;
			break;
		case 'k':	/* km along great circle */
			*distance_func = GMT_great_circle_dist;
			*d_scale = project_info.DIST_KM_PR_DEG;
			break;
		case 'K':	/* km along geodesic */
			*distance_func = (GMT_IS_SPHERICAL) ? GMT_great_circle_dist : GMT_geodesic_dist_meter;
			*d_scale = (GMT_IS_SPHERICAL) ? project_info.DIST_KM_PR_DEG : 0.001;
			break;
		case 'm':	/* Miles along great circle */
			*distance_func = GMT_great_circle_dist;
			*d_scale = project_info.DIST_M_PR_DEG / METERS_IN_A_MILE;
			break;
		case 'M':	/* Miles along geodesic */
			*distance_func = (GMT_IS_SPHERICAL) ? GMT_great_circle_dist : GMT_geodesic_dist_meter;
			*d_scale = ((GMT_IS_SPHERICAL) ? project_info.DIST_M_PR_DEG : 1.0) / METERS_IN_A_MILE;
			break;
		case 'n':	/* Nautical miles along great circle */
			*distance_func = GMT_great_circle_dist;
			*d_scale = project_info.DIST_M_PR_DEG / METERS_IN_A_NAUTICAL_MILE;
			break;
		case 'N':	/* Nautical miles along geodesic */
			*distance_func = (GMT_IS_SPHERICAL) ? GMT_great_circle_dist : GMT_geodesic_dist_meter;
			*d_scale = ((GMT_IS_SPHERICAL) ? project_info.DIST_M_PR_DEG : 1.0) / METERS_IN_A_NAUTICAL_MILE;
			break;
...
double GMT_set_label_offsets (GMT_LONG axis, double val0, double val1, struct GMT_PLOT_AXIS *A, GMT_LONG below, double annot_off[], double *label_off, GMT_LONG *annot_justify, GMT_LONG *label_justify, char *format)
{
	...
	if (axis == 0) {
		...
		if (A->unit[0] && gmtdefs.y_axis_type == 0) {	/* Accommodate extra width of annotation */
