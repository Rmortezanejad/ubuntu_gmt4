/*--------------------------------------------------------------------
 *	$Id: pshistogram.c 10289 2014-12-28 21:17:06Z pwessel $
 *
...
GMT_LONG fill_boxes (struct PSHISTOGRAM_INFO *F, double *data, GMT_LONG n) {
	...
	for (i = 0; i < n; i++) {
		ibox = (GMT_LONG)irint (floor( ( (data[i] - F->west) / F->box_width) + add_half));
		if (ibox < 0 || ibox >= F->n_boxes) continue;
		//if (gmtdefs.verbose == 2) fprintf (stderr, "Value %g landed in box %ld\n", data[i], ibox);
		F->boxh[ibox] ++;
		F->n_counted++;
	}
