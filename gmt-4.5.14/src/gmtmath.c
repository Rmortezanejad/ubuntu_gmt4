/*--------------------------------------------------------------------
 *	$Id: gmtmath.c 10289 2014-12-28 21:17:06Z pwessel $
 *
...
int main (int argc, char **argv)
{
	...
	GMT_LONG set_q = FALSE, read_stdin = FALSE, t_check_required = TRUE, no_C = TRUE, first_file = TRUE;
	...
	for (arg = 1; !error && arg < last_arg; arg++) {
		...
		if (op < GMTMATH_ARG_IS_OPERATOR) {	/* File name or factor */
			...
			if (op == GMTMATH_ARG_IS_T_MATRIX) {	/* Need to set up matrix of t-values */
				...
				for (j = 0; j < info.header.n_col; j++) if (no_C || !Ctrl->C.cols[j]) memcpy ((void *)stack[nstack][j], (void *)info.t_coordinates, (size_t)(info.header.n_row * sizeof (double)));
			}
				...
				for (j = 0; j < info.header.n_col; j++) if (no_C || !Ctrl->C.cols[j]) memcpy ((void *)stack[nstack][j], (void *)info.tn_coordinates, (size_t)(info.header.n_row * sizeof (double)));
			}
				...
				if (!strcmp (argv[arg], "STDIN")) {	/* stdin file */
					...
					for (j = 0; j < info.header.n_col; j++) if (no_C || !Ctrl->C.cols[j]) memcpy ((void *)stack[nstack][j], (void *)stdin_stack[j], (size_t)(info.header.n_row * sizeof (double)));
				}
				else {
					if (gmtdefs.verbose) fprintf (stderr, "%s ", argv[arg]);
					GMT_read_table (&info, argv[arg], &tbl[nstack], &stack[nstack], Ctrl->N.tcol, FALSE);
					/* Zero out any columns to be skipped via -C */
					if (!first_file) {
						for (j = 0; j < info.header.n_col; j++) if (!no_C || Ctrl->C.cols[j]) memset ((void *)stack[nstack][j], 0, (size_t)(info.header.n_row * sizeof (double)));
					}
					first_file = FALSE;
				}
