/*--------------------------------------------------------------------
 *	$Id: gmt_globals.h 10289 2014-12-28 21:17:06Z pwessel $
 *
...
struct GMT_DEFAULTS gmtdefs = {		/* Initial default values */
	...
	FALSE,			/* PORTRAIT */
	{612.0, 792.0},		/* PAPER_MEDIA (US Letter) */
	...
	35,			/* PS_MITER_LIMIT (0 = PS default, or 1-180) */
...
GMT_LONG GMT_N_FONTS;				/* Total number of fonts returned by GMT_init_fonts */
...
GMT_LONG GMT_x_status_new;				/* Tells us what quadrant old and new points are in */
GMT_LONG GMT_y_status_new;
GMT_LONG GMT_x_status_old;
GMT_LONG GMT_y_status_old;
GMT_LONG GMT_corner = 0;
GMT_LONG GMT_on_border_is_outside = FALSE;	/* TRUE if a point exactly on the map border shoud be considered outside the map */
GMT_LONG GMT_world_map = FALSE;			/* TRUE if map has 360 degrees of longitude range */
GMT_LONG GMT_world_map_tm = FALSE;		/* TRUE if GMT_TM map is global? */
GMT_LONG GMT_lon_wrap = FALSE;			/* TRUE if wrapping of longitude over 360 degrees is allowed */
...
PFL GMT_outside;				/* pointer to function checking if a lon/lat point is outside map */
PFL GMT_crossing;				/* pointer to functions returning crossover point at boundary */
PFL GMT_overlap;				/* pointer to function checking for overlap between 2 regions */
PFL GMT_map_clip;				/* pointer to functions that clip a polygon to fit inside map */
...
GMT_LONG GMT_z_periodic = FALSE;			/* TRUE if grid values are 0-360 degrees (phases etc) */
PFL GMT_wrap_around_check;			/* Does x or y wrap checks */
PFL GMT_map_jump;				/* TRUE if we jump in x or y */
...
GMT_LONG GMT_meridian_straight = FALSE;		/* TRUE if meridians plot as straight lines */
GMT_LONG GMT_parallel_straight = FALSE;		/* TRUE if parallels plot as straight lines */
GMT_LONG GMT_coastline = FALSE;			/* TRUE if parallels plot as straight lines */
GMT_LONG GMT_3D_mode = 3;			/* Determines if we draw fore and/or back 3-D box lines [Default is both] */
...
GMT_LONG GMT_n_colors = 0;			/* Number of colors in CPT lookup table */
GMT_LONG GMT_cpt_flags = 0;			/* Flags controling use of BFN colors */
GMT_LONG GMT_gray;			/* TRUE if only grayshades are needed */
GMT_LONG GMT_b_and_w;			/* TRUE if only black and white are needed */
GMT_LONG GMT_continuous;			/* TRUE if continuous color tables have been given */
GMT_LONG GMT_cpt_pattern = FALSE;	/* TRUE if cpt file contains any patterns */
GMT_LONG GMT_cpt_skip = FALSE;		/* TRUE if current z-slice is to be skipped */
#ifdef GMT_CPT2	
GMT_LONG GMT_categorical = FALSE;	/* TRUE if CPT applies to categorical data */
#endif
/*--------------------------------------------------------------------*/
...
PFL GMT_forward, GMT_inverse;		/* Pointers to the selected mapping functions */
PFL GMT_x_forward, GMT_x_inverse;	/* Pointers to the selected linear functions */
PFL GMT_y_forward, GMT_y_inverse;	/* Pointers to the selected linear functions */
PFL GMT_z_forward, GMT_z_inverse;	/* Pointers to the selected linear functions */
...
PFL GMT_input;				/* Pointer to function reading ascii or binary tables */
PFL GMT_input_ascii;			/* Pointer to function reading ascii tables only */
PFL GMT_output;				/* Pointer to function writing ascii or binary tables */
PFL GMT_output_ascii;			/* Pointer to function writing ascii tables only */
...
GMT_LONG GMT_n_file_suffix;
PFL GMT_io_readinfo[GMT_N_GRD_FORMATS];
PFL GMT_io_updateinfo[GMT_N_GRD_FORMATS];
PFL GMT_io_writeinfo[GMT_N_GRD_FORMATS];
PFL GMT_io_readgrd[GMT_N_GRD_FORMATS];
PFL GMT_io_writegrd[GMT_N_GRD_FORMATS];
double *GMT_file_scale, *GMT_file_offset, *GMT_file_nan;
GMT_LONG *GMT_file_id;
char **GMT_file_suffix;
double GMT_curr_rec[GMT_MAX_COLUMNS], GMT_prev_rec[GMT_MAX_COLUMNS];
GMT_LONG GMT_pad[4] = {0, 0, 0, 0};
GMT_LONG GMT_inc_code[2] = {0, 0};
struct GMT_GRD_INFO GMT_grd_info;
...
GMT_LONG GMT_n_custom_symbols = 0;
...
GMT_LONG GMT_min_meminc = GMT_MIN_MEMINC;
GMT_LONG GMT_max_meminc = GMT_MAX_MEMINC;
