/*	$Id: gshhgtograss.c 10282 2014-11-17 01:59:16Z pwessel $
*
...
int main (int argc, char **argv)
{
	...
	int shore_levels = 5;
	uint32_t k32, max_id=0;
	FILE *fp = NULL, *ascii_fp = NULL, *att1_fp = NULL, *att2_fp = NULL;
	int max = 270000000, level, version, greenwich, shorelines;
	...
	*strrchr(dataname,056)= 0;
	...
	shorelines = strstr(dataname,"gshhg") ? 1 : 0;
	...
	n_read = fread((void *)&h, (size_t)sizeof (struct GSHHG_HEADER), (size_t)1, fp);
	...
	while (n_read == 1) {
		...
		if( ( w <= maxx && e >= minx ) && ( s <= maxy && n >= miny ) ){
			...
			*/
			...
			fseek (fp, (long)(h.n * sizeof(struct GSHHG_POINT)), SEEK_CUR);
		}
...
/* Make dummy functions so gshhgtograss will link under WIN32 */
